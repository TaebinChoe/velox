...
class MockCursor:
    ...
    def _get_table(self, name):
        ...
        mapping = {
            ...
            "file_node": "files",
            "netflow_node": "netflows"
        }
        mapped_name = mapping.get(name, name)
        if mapped_name not in self._tables:
            filepath = os.path.join(self.data_dir, f"{mapped_name}.parquet")
            if not os.path.exists(filepath):
                filepath = os.path.join("./data", f"{mapped_name}.parquet")
            if os.path.exists(filepath):
                self._tables[mapped_name] = pd.read_parquet(filepath)
            else:
                raise FileNotFoundError(f"Mock DB: Parquet file '{filepath}' not found.")
        return self._tables[mapped_name]
    ...
    def execute(self, sql, params=None):
        ...
        m_all = re.match(r"(?i)select\s+\*\s+from\s+(\w+)(?:\s+where\s+(.+))?", sql)
        if m_all:
            table_name = m_all.group(1)
            df = self._get_table(table_name)
            where_clause = m_all.group(2)
            if where_clause:
                order_by = None
                if "ORDER BY" in where_clause.upper():
                    where_clause, order_by = re.split(r"(?i)ORDER BY", where_clause, 1)
                
                # Filter timestamp_rec
                if "BETWEEN" in where_clause.upper():
                    m_between = re.search(r"(?i)timestamp_rec\s+BETWEEN\s+'?(\d+)'?\s+AND\s+'?(\d+)'?", where_clause)
                    if m_between:
                        t_min, t_max = int(m_between.group(1)), int(m_between.group(2))
                        df = df[(df['timestamp_rec'] >= t_min) & (df['timestamp_rec'] <= t_max)]
                else:
                    nums = [int(s.replace("'", "")) for s in re.findall(r"\d+|'\d+'", where_clause)]
                    if len(nums) >= 2:
                        df = df[(df['timestamp_rec'] > nums[0]) & (df['timestamp_rec'] < nums[1])]
                
                # Check for IN clause for src_index_id
                m_src_in = re.search(r"(?i)src_index_id\s+IN\s*\((.*?)\)", where_clause)
                if m_src_in:
                    items = [x.strip().replace("'", "").replace('"', '') for x in m_src_in.group(1).split(',')]
                    df = df[df['src_index_id'].astype(str).isin(items)]
                
                # Check for IN clause for dst_index_id
                m_dst_in = re.search(r"(?i)dst_index_id\s+IN\s*\((.*?)\)", where_clause)
                if m_dst_in:
                    items = [x.strip().replace("'", "").replace('"', '') for x in m_dst_in.group(1).split(',')]
                    df = df[df['dst_index_id'].astype(str).isin(items)]

                if order_by:
                    cols = [c.strip().replace(";", "").split()[0] for c in order_by.split(',')]
                    df = df.sort_values(by=cols)
            
            self.results = [tuple(x) for x in df.to_numpy()]
            return

        m_cols = re.match(r"(?i)select\s+(.+?)\s+from\s+(\w+)", sql)
        if m_cols:
            cols_str = m_cols.group(1)
            table_name = m_cols.group(2)
            df = self._get_table(table_name)
            cols = [c.strip().replace(";", "") for c in cols_str.split(',')]
            self.results = [tuple(x) for x in df[cols].to_numpy()]
            return

        print(f"Mock DB: Unknown query format: '{sql}'")
        self.results = []
...
class MockConnection:
    def __init__(self, data_dir="./data"):
        self.data_dir = data_dir
    def cursor(self):
        return MockCursor(self.data_dir)
    def commit(self):
        pass
    def close(self):
        pass
...
def init_database_connection(cfg):
    # Retrieve data_dir from configuration or default to ./data
    data_dir = getattr(cfg.database, "data_dir", "./data")
    connect = MockConnection(data_dir)
    cur = connect.cursor()
    return cur, connect
