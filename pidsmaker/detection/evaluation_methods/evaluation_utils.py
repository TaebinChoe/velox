...
def plot_scores_with_paths_node_level(
    ...
    for n in nodes:
        node_info = node_to_path.get(n, {"path": "Unknown", "type": "unknown"})
        types.append(node_info["type"])
        path = node_info["path"]
        if path == "None":
            paths.append(node_info["cmd"] if "cmd" in node_info else path)
        else:
            paths.append(path)
...
def plot_scores_with_paths_edge_level(
    ...
    for src, dst, *_ in edges:
        src = int(src)
        dst = int(dst)
        src_info = node_to_path.get(src, {"path": "Unknown", "type": "unknown"})
        dst_info = node_to_path.get(dst, {"path": "Unknown", "type": "unknown"})
        src_type = src_info["type"]
        dst_type = dst_info["type"]
        types.append(src_type)

        path_src = (
            src_info["path"] + ", " + src_info["cmd"]
            if src_type == "subject" and "cmd" in src_info
            else src_info["path"]
        )
        path_dst = (
            dst_info["path"] + ", " + dst_info["cmd"]
            if dst_type == "subject" and "cmd" in dst_info
            else dst_info["path"]
        )
...
def plot_detected_attacks_vs_precision(scores, nodes, node2attacks, labels, out_file):
    ...
    for i, node in enumerate(sorted_nodes):
        ...
        precision = tp / (tp + fp)
        detected_attacks_percentage = (len(detected_attacks) / total_attacks) * 100 if total_attacks > 0 else 0.0
...
def compute_discrimination_score(pred_scores, nodes, node2attacks, y_truth, k=10):
    ...
    for att in set().union(*node2attacks.values()):
